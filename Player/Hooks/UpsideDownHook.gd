...
var elasticity = .10
var grapple_boost = Vector2(1.2,1.5)
var x_speed
...
func set_aim_rotation():
	if grappled:
		...
		if just_released:
				$GrappleCast.rotation_degrees = x_speed/max_speed*(45)
				just_released = false
		else:
			$GrappleCast.rotation_degrees = lerp($GrappleCast.rotation_degrees, x_speed/max_speed*(45), 0.2)
...
func get_reel_speed():
	...
	return Vector2(0,-25)+Vector2(1,-1)*grapple_vector.normalized()*reel_in_speed
