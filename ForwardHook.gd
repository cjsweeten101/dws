...
var elasticity = .10
var grapple_boost = Vector2(2.0,1.0)
var x_speed
...
func set_aim_rotation():
	if grappled:
		...
		if just_released:
				$GrappleCast.rotation_degrees = 222 + x_speed/max_speed*(45)
				just_released = false
		else:
			$GrappleCast.rotation_degrees = lerp($GrappleCast.rotation_degrees, 222 + x_speed/max_speed*(45), 0.2)
...
func get_reel_speed():
	...
	if grapple_vector.length() > grapple_length+10 and !reel_first_pass:
		release()
		return Vector2(0,0)
	if reel_first_pass:
	...
	var norm_grapple = grapple_vector.normalized()
	#Change to boos more if grappling close to flat.....		
	return Vector2(0,-40)+grapple_vector.normalized()*reel_in_speed
